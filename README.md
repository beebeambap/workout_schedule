# 레슨핏 (workout_schedule)

트레이너·강사를 위한 레슨 스케줄 + 회원 관리 웹앱(PWA). 회원 이름·날짜·시간을 입력하면 캘린더에 자동 등록되고, 회원별 주간/월간 캘린더를 JPG로 내보낼 수 있습니다. 다기기 동기화(Supabase) + 앱 잠금(PIN) + 한국 공휴일 표시 지원.
